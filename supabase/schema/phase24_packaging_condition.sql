-- ============================================
-- PATWADI — Packaging condition on pickup custody events
-- Run AFTER phase23_terms_accepted.sql
-- ============================================

ALTER TABLE custody_events
  ADD COLUMN IF NOT EXISTS packaging_condition text DEFAULT NULL
  CHECK (packaging_condition IN ('acceptable', 'risk_acknowledged_by_customer'));

DROP FUNCTION IF EXISTS acknowledge_handoff_atomic(UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION);
...
BEGIN
  ...
  IF p_step = 'customer_to_lmp' THEN
    IF p_packaging_condition IS NULL OR p_packaging_condition NOT IN ('acceptable', 'risk_acknowledged_by_customer') THEN
      RAISE EXCEPTION 'PACKAGING_CONDITION_REQUIRED';
    END IF;
  ELSIF p_packaging_condition IS NOT NULL THEN
    RAISE EXCEPTION 'PACKAGING_CONDITION_NOT_ALLOWED';
  END IF;
...
REVOKE ALL ON FUNCTION acknowledge_handoff_atomic(UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION acknowledge_handoff_atomic(UUID, UUID, TEXT, TEXT, UUID, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, DOUBLE PRECISION, TEXT) TO service_role;
